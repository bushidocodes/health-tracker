...
- **typeahead.js 0.11.1 is abandoned (last release 2015) and has documented jQuery 3.x compatibility issues** (see [twitter/typeahead.js#1679](https://github.com/twitter/typeahead.js/issues/1679)). The app currently loads jQuery 3.7.1 alongside typeahead.js 0.11.1. In practice the known incompatibilities (around `.val()` and event handling) do not visibly break the current usage, but the combination is unsupported and may fail under edge cases or future jQuery updates. Migration path: replace with a maintained fork such as [corejs-typeahead](https://github.com/corejavascript/typeahead.js) (drop-in API-compatible) or a different autocomplete library (Awesomplete, native `<datalist>`).
